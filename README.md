...
- **消息管理**：用户可查看系统消息
- **用户头像上传**：用户可上传、更新个人头像，系统使用MinIO存储并生成预签名URL

#### 用户头像功能：
- **头像上传**：支持JPG/PNG/GIF格式图片上传，大小限制为2MB
- **头像存储**：使用MinIO对象存储服务，按用户ID隔离存储
- **头像访问**：使用预签名URL技术，有效期24小时，保障安全访问
- **默认头像**：用户未上传头像时使用系统默认头像
- **头像更新**：用户上传新头像时自动删除旧头像，保持存储空间清洁
...
11. 用户头像上传功能支持JPG/PNG/GIF格式，大小限制为2MB，使用MinIO存储并生成预签名URL
12. 处理大整数ID时，前端应使用字符串类型，避免JavaScript精度丢失问题

## 最近更新

### 2025-10-20 保护区介绍更新功能修复

1. 修复了保护区介绍更新功能中的用户ID传递问题，确保在文件上传时能正确使用用户ID
2. 优化了文件处理逻辑，添加了详细的日志记录，方便排查问题
3. 修复了JWT令牌解析过程中的用户ID类型转换问题
4. 改进了控制器与服务层之间的数据传递，确保用户ID正确设置到DTO的createBy和updateBy属性中
5. 解决了文件记录中的upload_user_id、create_by和update_by字段设置错误的问题
6. 优化了文件删除逻辑，确保在更新操作时，如果有新文件上传，则删除原有的所有文件

### 2025-10-14 用户头像上传功能

1. 实现了用户头像上传功能，支持JPG/PNG/GIF格式，大小限制为2MB
2. 使用MinIO对象存储服务存储头像，按用户ID隔离存储
3. 使用预签名URL技术访问头像，有效期24小时，保障安全访问
4. 修复了JavaScript数字精度问题，将大整数ID改为字符串类型
5. 优化了JWT认证设计，解决了拦截器和控制器认证逻辑的矛盾
6. 添加了详细的技术文档：
   - `avatar-upload-flow.md`：用户头像上传技术流程
   - `jwt-auth-explanation.md`：JWT认证与头像API的设计考量
   - `number-precision-issue.md`：JavaScript数字精度问题及解决方案
   - `id-generation-strategy.md`：用户ID生成策略说明
